...
import { getGrievances, getOdc, getOdcNotForCitizen } from "@/apicalls/citizen";
...
export function OnDemandCollection() {
    ...
    async (
      ...
      try {
        ...
        if (user?.roles.includes('citizen')) {
          response = await getOdc(page, limit, {
            ...opts,
          });
        } else {
          response = await getOdcNotForCitizen(page, limit, {
            ...opts,
          });
        }
  ...
  const fetchAllData = useCallback(async () => {
    try {
      ...
      if (user?.roles.includes('citizen')) {
        response = await getOdc(1, 10000);
      } else {
        response = await getOdcNotForCitizen(1, 10000);
      }
  ...
  return (
    <div className="bg-background rounded-lg">
      ...
      <div className="flex items-center justify-between p-4">
        <h2 className="text-2xl font-bold">On Demand Collection</h2>
      </div>
      ...
      <div className="flex flex-col gap-4 p-4">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 flex-wrap">
          ...
          /> */}
          {user?.roles.includes('citizen') && (<SubmissionForm setRefetch={setRefetch} />)}
        </div>
      </div>
            ...
            {isLoading ? (
              ...
              <TableRow>
                <TableCell
                  ...
                  className="text-center py-4 text-gray-500"
                >
                  No on demand collection data found.
                </TableCell>
              </TableRow>
            ) : (
              data.map((item, index) => (
                ...
                >
                  ...
                  <TableCell className="px-4 py-3 text-xs">
                    {formatToIST(String(item?.created_at)) || "-"}
                  </TableCell>
                </TableRow>
              ))
            )}
