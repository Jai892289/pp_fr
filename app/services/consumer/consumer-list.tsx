...
export function ConsumerList() {
  const [data, setData] = useState<ConsumerDetail[]>([]);
  ...
  const fetchData = useCallback(
    async (
      ...
      try {
        ...
        const response = await getConsumerDataList(page, limit, {
          ...opts,
        });
  ...
  const fetchAllData = useCallback(async () => {
    try {
      ...
      const response = await getConsumerDataList(1, 10000);
  ...
  return (
    <div className="bg-background rounded-lg">
      ...
      <div className="flex items-center justify-between p-4">
        <h2 className="text-2xl font-bold">Consumer Report</h2>
      </div>
          ...
          <ExportDropdown
            config={{
              ...
              columns: [
                ...
                { key: "current_waste_collection_status", label: "Status" },
              ],
            }}
          />
          ...
          <TableBody>
            {isLoading ? (
              ...
              data.map((item, index) => (
                <TableRow
                  key={item.id}
                  onClick={() => handleView(String(item?.integrated_property_id))}
                ...
                >
                  ...
                  <TableCell className="px-4 py-3 text-xs">
                    {item?.integrated_property_id || "-"}
                  </TableCell>
                  <TableCell className="px-4 py-3 text-xs">
                    {item?.integrated_owner_name || "-"}
                  </TableCell>
                  {/* <TableCell className="px-4 py-3 text-xs">
                    {item?.integrated_owner_name || "-"}
                  </TableCell>
                  ...
                  <TableCell className="px-4 py-3 text-xs">
                    {item?.mobile ? maskNumber(item?.mobile) : "-"}
                  </TableCell>
                  <TableCell className="px-4 py-3 text-xs">
                  ...
                  <TableCell className="px-4 py-3 text-xs">
                    {(() => {
                      const rawVal = item?.is_tagged;

                      // ✅ safely normalize anything (string | number | boolean | null)
                      const isTrue =
