...
import { RoleDialog } from '@/components/dialogs/role-master/role_dialog';
import { ToggleConfirmationDialog } from '@/components/dialogs/role-master/toggle_role';
import { getAllRoleApi, toggleRoleApi } from '@/apicalls/panelSetup';
...
export function RoleMasterTable() {
  ...
  const { toast } = useToast()
    ...
    const fetchAllData = useCallback(async () => {
      try {
        ...
        const response = await getAllRoleApi(1, 1000) // Fetch up to 1000 records
   ...
   const fetchData = useCallback(async (page: number = 1, limit: number = itemsPerPage) => {
     try {
       setIsLoading(true);
       const response = await getAllRoleApi(page, limit);
  ...
  const handleToggleConfirm = async () => {
    ...
    try {
      await toggleRoleApi(selectedRole.id)
      await fetchData(currentPage, itemsPerPage)
      const action = selectedRole.recstatus === 1 ? "deactivated" : "activated"
      toast({
        title: "Success",
    ...
    <>
      <div className="bg-background rounded-lg">
        ...
        <div className="flex items-center justify-between p-4">
          <div className="flex items-center gap-4">
            <h2 className="text-2xl font-bold pl-2">Role Master</h2>
          </div>
      ...
      {/* Toggle Confirmation Dialog */}
        <ToggleConfirmationDialog
          ...
          title={`${selectedRole?.recstatus === 1 ? "Deactivate" : "Activate"} Module`}
          description={`Are you sure you want to ${selectedRole?.recstatus === 1 ? "deactivate" : "activate"} "${selectedRole?.name}"?`}
          onConfirm={handleToggleConfirm}
          currentStatus={selectedRole?.recstatus || 0}
      />
