...
import { Ward, WardApiResponse } from '@/types/wardpanel';
import {WardDialog} from '@/components/dialogs/ward-menu/ward-dialog';
import {ToggleConfirmationDialog} from '@/components/dialogs/ward-menu/toggle-ward-dialog';
import { getAllWards, toggleWard } from '@/apicalls/panelSetup';
...
export function WardTable() {
  ...
  const [allData, setAllData] = useState<Ward[]>([])
  ...
  const { toast } = useToast()
    ...
    const fetchAllData = useCallback(async () => {
      try {
        ...
        const response = await getAllWards(1, 1000) // Fetch up to 1000 records
    ...
    const fetchData = useCallback(async (page: number = 1, limit: number = itemsPerPage) => {
      try {
        setIsLoading(true);
        const response = await getAllWards(page, limit);
  ...
  const handleToggleConfirm = async () => {
    ...
    try {
      await toggleWard(selectedWard.id)
      await fetchData(currentPage, itemsPerPage)
      const action = selectedWard.recstatus === 1 ? "deactivated" : "activated"
      toast({
        title: "Success",
    ...
    <>
      <div className="bg-background rounded-lg">
        ...
        <div className="flex items-center justify-between p-4">
          <div className="flex items-center gap-4">
            <h2 className="text-2xl font-bold pl-2">Ward Master</h2>
          </div>
          ...
          <Table>
            ...
            <TableBody>
              {isLoading || data.length === 0 ? (
                ...
                filteredULBTypeData.map((ward, index) => (
                  <TableRow key={ward.id} className="border-b hover:bg-gray-50/30 transition-colors">
                    ...
                    <TableCell className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-medium ">{ward.ward_no}</span>
                      </div>
                    </TableCell>
                    ...
                    <TableCell className="px-4 py-3">
                      <span className="text-sm font-medium ">{ward.ward_code}</span>
                    </TableCell>

                    <TableCell className="px-4 py-3">
                      <span className="text-sm font-medium ">{ward.area || '-'}</span>
                    </TableCell>
                    <TableCell className="px-4 py-3">
                      <span className="text-sm font-medium ">Zone {ward.zone_circle_id}</span>
                    </TableCell>

                    <TableCell className="px-4 py-3">
                      <span
                        className={`inline-flex items-center px-2 py-1 rounded-sm text-xs font-medium ${
                          ward.recstatus === 1 ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"
                        }`}
                      >
                    ...
                    <TableCell className="px-4 py-3">
                      <div className="flex items-center justify-end gap-8">
                        ...
                        <div>
                          <Switch
                            checked={ward.recstatus === 1}
                            onCheckedChange={() => handleToggle(ward)}
                            disabled={toggleLoading === ward.id}
                            className="data-[state=checked]:bg-green-600 data-[state=unchecked]:bg-gray-300 cursor-pointer"
                          />
                          {toggleLoading === ward.id && (
      ...
      {/* Toggle Confirmation Dialog */}
        <ToggleConfirmationDialog
          ...
          title={`${selectedWard?.recstatus === 1 ? "Deactivate" : "Activate"} Module`}
          description={`Are you sure you want to ${selectedWard?.recstatus === 1 ? "deactivate" : "activate"} "${selectedWard?.name}"?`}
          onConfirm={handleToggleConfirm}
          currentStatus={selectedWard?.recstatus || 0}
      />
