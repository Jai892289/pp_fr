...
import { Permission, PermissionApiResponse, PermissionListResponse } from '@/types/permission';
import {PermissionDialog} from '@/components/dialogs/permission/permission-dialog';
import { TogglePermissionDialog } from '@/components/dialogs/permission/toggle-permission-dialog';
import { togglePermission } from '@/apicalls/panelSetup';
...
export function PermissionTable({ getAllPermissions }: { getAllPermissions: (page?: number, limit?: number) => Promise<PermissionApiResponse> }) {
   const [data, setData] = useState<Permission[]>([])
  const [currentPage, setCurrentPage] = useState(1)
  const [allData, setAllData] = useState<Permission[]>([])
  ...
  const { toast } = useToast()
    ...
    const fetchAllData = useCallback(async () => {
      try {
        ...
        const response = await getAllPermissions(1, 10000) // Fetch up to 10,000 records
  ...
  const fetchData = useCallback(async (page: number = 1, limit: number = itemsPerPage) => {
    try {
      ...
      if (response && response.data) {
        ...
        setData(permissions);
        setAllData(permissions);
        setTotalItems(pagination.total || permissions.length);
  ...
  const handleToggleConfirm = async () => {
    ...
    try {
      await togglePermission(selectedPermission.id)
      await fetchData(currentPage, itemsPerPage)
      const action = selectedPermission.recstatus === 1 ? "deactivated" : "activated"
      toast({
        title: "Success",
  ...
  return (
    <>
      <div className="bg-background rounded-lg">
        ...
        <div className="flex items-center justify-between p-4">
          <div className="flex items-center gap-4">
            <h2 className="text-2xl font-bold pl-2">Permission List</h2>
          </div>
      ...
      <TogglePermissionDialog
        ...
        title={`${selectedPermission?.recstatus === 1 ? "Deactivate" : "Activate"} Permission`}
        description={`Are you sure you want to ${selectedPermission?.recstatus === 1 ? "deactivate" : "activate"} "${selectedPermission?.name}"?`}
        onConfirm={handleToggleConfirm}
        currentStatus={selectedPermission?.recstatus || 0}
      />
