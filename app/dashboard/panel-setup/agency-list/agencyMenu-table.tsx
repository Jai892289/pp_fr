...
import { toggleAgency } from '@/apicalls/panelSetup';
import { Agency } from '@/types/agency';
import { AgencyApiResponse } from '@/types/api';
import { AgencyDialog } from '@/components/dialogs/agency-menu/agency-dialog';
...
export function AgencyMenuTable({ getAllAgency }: { getAllAgency: (page?: number, limit?: number) => Promise<AgencyApiResponse> }) {
   const [data, setData] = useState<Agency[]>([])
  const [currentPage, setCurrentPage] = useState(1)
  const [allData, setAllData] = useState<Agency[]>([])
  ...
  const fetchAllData = useCallback(async () => {
    try {
      ...
      const response = await getAllAgency(1, 10000) // Fetch up to 10,000 records
  ...
  const fetchData = useCallback(async (page: number = 1, limit: number = itemsPerPage) => {
    try {
      setIsLoading(true);
      const response = await getAllAgency(page, limit);
      ...
      if (response && response.data) {
        ...
        setTotalItems(responseData.pagination || 0);
        setTotalPages(responseData.totalPages || 0);
        setCurrentPage(responseData.page || 1);
        setItemsPerPage(responseData.limit || 10);
      } else {
  ...
  const handleToggleConfirm = async () => {
    ...
    try {
      await toggleAgency(selectedAgency.id)
      await fetchData(currentPage, itemsPerPage)
      const action = selectedAgency.recstatus === 1 ? "deactivated" : "activated"
      toast({
        title: "Success",
  ...
  return (
    <>
      <div className="bg-background rounded-lg">
        ...
        <div className="flex items-center justify-between p-4">
          <div className="flex items-center gap-4">
            <h2 className="text-2xl font-bold pl-2">Agency List</h2>
          </div>
              ...
              {isLoading || data.length === 0 ? (
                ...
                filteredULBTypeData.map((agency, index) => (
                  <TableRow key={agency.id} className="border-b hover:bg-gray-50/30 transition-colors">
                    ...
                    <TableCell className="px-4 py-3">
                      <span className="text-sm font-medium ">{agency.ulb_id}</span>
                    </TableCell>
      ...
      <ToggleConfirmationDialog
        ...
        title={`${selectedAgency?.recstatus === 1 ? "Deactivate" : "Activate"} Agency`}
        description={`Are you sure you want to ${selectedAgency?.recstatus === 1 ? "deactivate" : "activate"} "${selectedAgency?.name}"?`}
        onConfirm={handleToggleConfirm}
        currentStatus={selectedAgency?.recstatus || 0}
      />
