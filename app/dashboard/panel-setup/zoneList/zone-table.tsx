...
import { Zone, ZoneApiResponse } from '@/types/zonepanel';
import { ZoneDialog } from '@/components/dialogs/zone-menu/zone-dialog';
import { ToggleConfirmationDialog } from '@/components/dialogs/zone-menu/toggle-zone-dialog';
import { toggleZone, getAllZones } from '@/apicalls/panelSetup';
...
export function ZoneTable() {
  ...
  const [allData, setAllData] = useState<Zone[]>([])
  ...
  const { toast } = useToast()
    ...
    const fetchAllData = useCallback(async () => {
      try {
        ...
        const response = await getAllZones(1, 1000) // Fetch up to 1000 records
    ...
    const fetchData = useCallback(async (page: number = 1, limit: number = itemsPerPage) => {
      try {
        setIsLoading(true);
        const response = await getAllZones(page, limit);
  ...
  const handleToggleConfirm = async () => {
    ...
    try {
      await toggleZone(selectedZone.id)
      await fetchData(currentPage, itemsPerPage)
      const action = selectedZone.recstatus === 1 ? "deactivated" : "activated"
      toast({
        title: "Success",
  ...
  return (
    <>
      <div className="bg-background rounded-lg">
        ...
        <div className="flex items-center justify-between p-4">
          <div className="flex items-center gap-4">
            <h2 className="text-2xl font-bold pl-2">Zone List</h2>
          </div>
          ...
          <Table>
            ...
            <TableBody>
              {isLoading || data.length === 0 ? (
                ...
                filteredULBTypeData.map((zone, index) => (
                  <TableRow key={zone.id} className="border-b hover:bg-gray-50/30 transition-colors">
                    ...
                    <TableCell className="px-4 py-3">
                      <span className="text-sm font-medium ">{zone.zonecode}</span>
                    </TableCell>

                    <TableCell className="px-4 py-3">
                      <span className="text-sm font-medium ">{zone.ulb_id}</span>
                    </TableCell>

                    <TableCell className="px-4 py-3">
                      <span
                        className={`inline-flex items-center px-2 py-1 rounded-sm text-xs font-medium ${
                          zone.recstatus === 1 ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"
                        }`}
                      >
                        {zone.recstatus === 1 ? "Active" : "Inactive"}
                      </span>
                    </TableCell>
      ...
      <ToggleConfirmationDialog
        ...
        title={`${selectedZone?.recstatus === 1 ? "Deactivate" : "Activate"} Zone`}
        description={`Are you sure you want to ${selectedZone?.recstatus === 1 ? "deactivate" : "activate"} "${selectedZone?.name}"?`}
        onConfirm={handleToggleConfirm}
        currentStatus={selectedZone?.recstatus || 0}
      />
